...
export default function LeaderboardPage() {
    ...
    const [activeFilter, setActiveFilter] = useState<FilterKey>('total_wins');
    ...
    const loadInitialData = async () => {
        ...
        const { data: events } = await supabase.from('events').select('event_date').order('event_date', { ascending: false });
        if (events) {
                    ...
                    <div className="relative">
                        ...
                        >
                            <div className="flex items-center gap-2">
                                <Icon icon="solar:calendar-linear" className="text-orange-500" />
                                <span>{selectedMonth === 'all' ? 'ทั้งหมด' : new Date(selectedMonth + '-01').toLocaleDateString('th-TH', { month: 'long', year: 'numeric' })}</span>
                            </div>
